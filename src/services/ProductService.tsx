...
export const useProducts = (category_slug: string, limit = 5, page = 1) => {
  let linkApi = "";
  if (category_slug) {
    linkApi =
  ...
  const fetchProducts = () =>
    axios.get<ApiResponse>(linkApi).then((response) => response.data.data);
...
export const useProductBySlug = (
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR<Product>(linkApi, fetchProducts, {
    fallbackData: initialData,
    // Refetch when the data is stale (outdated)
    revalidateIfStale: false, // Refetch data when it's stale
    // Refetch when the page is focused again
